...
import (
	...
	"github.com/adnannzz/centmail/internal/utils"
...
func install(lastVer string, db *sqlx.DB, fs stuffbin.FileSystem, prompt, idempotent bool) {
	...
	if !idempotent {
		...
		fmt.Printf("** IMPORTANT: This will wipe existing CentMail tables and types in the DB '%s' **",
	...
	var (
		user     = os.Getenv("CENTMAIL_ADMIN_USER")
		password = os.Getenv("CENTMAIL_ADMIN_PASSWORD")
		apiUser  = os.Getenv("CENTMAIL_ADMIN_API_USER")
	...
	if user != "" && password != "" {
		if len(user) < 3 || len(password) < 8 {
			lo.Fatal("CENTMAIL_ADMIN_USER should be min 3 chars and CENTMAIL_ADMIN_PASSWORD should be min 8 chars")
		}
	...
	if apiUser != "" {
		if !hasUser {
			lo.Fatal("CENTMAIL_ADMIN_API_USER requires CENTMAIL_ADMIN_USER and CENTMAIL_ADMIN_PASSWORD to be set")
		}

		if len(apiUser) < 3 {
			lo.Fatal("CENTMAIL_ADMIN_API_USER should be min 3 chars")
		}
...
func installCampaign(campTplID, archiveTplID int, q *models.Queries) {
	...
	if _, err := q.CreateCampaign.Exec(uuid.Must(uuid.NewV4()),
		...
		<p>For help, refer to the documentation.</p>
		`,
		nil,
		"richtext",
		nil,
...
func installUser(username, password, apiUsername string, q *models.Queries) {
	...
	if _, err := q.CreateUser.Exec(username, true, password, username+"@centmail", username, auth.RoleTypeUser, role.ID, nil, auth.UserStatusEnabled); err != nil {
	...
	if apiUsername != "" {
		...
		lo.Println("writing API token CENTMAIL_ADMIN_API_TOKEN to stderr")
		fmt.Fprintf(os.Stderr, "export CENTMAIL_ADMIN_API_TOKEN=\"%s\"\n", tk)
	}
}
