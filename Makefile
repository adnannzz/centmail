...
VERSION := $(or $(CENTMAIL_VERSION),$(shell git describe --tags --abbrev=0 2> /dev/null),$(shell grep -oP 'tag: \Kv\d+\.\d+\.\d+(-[a-zA-Z0-9.-]+)?' VERSION),"v0.0.0")
...
# Bundle all static assets including the JS frontend into the ./centmail binary
...
init-dev-docker: build-dev-docker ## Delete the docker containers including DB volumes.
	cd dev; \
	docker compose run --rm backend sh -c "make dist && ./centmail --install --idempotent --yes --config dev/config.toml"
